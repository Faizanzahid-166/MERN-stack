...
// ↳ Route: /profile
// Was: src/pages/Profile.jsx — lowercased.
// Fixed: setUser now exposed from AuthContext (added below).
...
function EditProfileTab({ user }) {
  ...
  const onSubmit = async (data) => {
    try {
      ...
      if (data.name !== user.name) { fd.append('name', data.name); hasChanges = true; }
      if (data.avatar?.[0])        { fd.append('avatar', data.avatar[0]); hasChanges = true; }
      if (!hasChanges) return toast.error('No changes detected');
      const res = await authAPI.updateProfile(fd);
      setUser(res.data.user || { ...user, name: data.name });
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
      <div className="bg-white dark:bg-gray-900 rounded-3xl border border-gray-100 dark:border-gray-800 p-6 shadow-sm">
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
          ...
          <div>
            <label className="block text-sm font-medium mb-1.5 text-gray-700 dark:text-gray-300">Profile Photo</label>
            <input
...
function BookmarksTab() {
  ...
  if (loading) return (
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
      {Array(3).fill(0).map((_, i) => <div key={i} className="h-56 skeleton rounded-2xl" />)}
    </div>
  );
    ...
    <div className="max-w-md">
      <div className="bg-white dark:bg-gray-900 rounded-3xl border border-gray-100 dark:border-gray-800 p-6 shadow-sm">
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
          ...
          <Input label="New Password" type="password" placeholder="Min. 6 characters" hint="At least 6 characters"
