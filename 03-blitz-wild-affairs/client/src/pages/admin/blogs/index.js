// src/pages/admin/blogs/index.js
// ↳ Route: /admin/blogs
// Was: src/pages/admin/BlogList.jsx — moved into blogs/ subfolder.
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-gray-100 dark:border-gray-800 text-left">
                {['Title', 'Category', 'Status', 'Views', 'Date', 'Actions'].map((h) => (
                  <th key={h} className="px-6 py-3 font-medium text-gray-500">{h}</th>
                ))}
              </tr>
            </thead>
            <tbody className="divide-y divide-gray-50 dark:divide-gray-800">
              {loading ? (
                ...
                >
                  <td className="px-6 py-4"><p className="font-medium truncate max-w-xs">{blog.title}</p></td>
                  <td className="px-6 py-4 text-gray-500">{blog.category || '—'}</td>
                  <td className="px-6 py-4">
                    <span className={`px-2 py-0.5 rounded-full text-xs font-medium ${blog.published ? 'bg-green-100 text-green-600' : 'bg-yellow-100 text-yellow-600'}`}>
                  ...
                  <td className="px-6 py-4">
                    <div className="flex items-center gap-3">
                      <Link href={`/admin/blogs/edit/${blog.id}`} className="text-brand-600 hover:underline font-medium">Edit</Link>
                      <Link href={`/blogs/${blog.slug}`} target="_blank" className="text-gray-400 hover:text-gray-600">View</Link>
