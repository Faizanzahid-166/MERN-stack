  ...
  return (
    ...
    >
      <div className="max-w-7xl mx-auto px-4 sm:px-6 h-16 flex items-center justify-between gap-6">
        ...
        {/* Logo */}
        <Link to="/" className="font-display font-black text-2xl text-brand-600 hover:opacity-90 transition-opacity flex-shrink-0">
          Blitz Wild Affairs
        </Link>
        ...
        <nav className="hidden md:flex items-center gap-7">
            ...
            <NavLink key={link.to} to={link.to} end={link.end}
              className={({ isActive }) =>
                `text-sm font-medium transition-colors ${
                  isActive ? 'text-brand-600' : 'text-gray-700 dark:text-gray-300 hover:text-brand-600'
                }`
              }
            >
              {link.label}
          ...
          {isAdmin && (
            <Link to="/admin" className="text-sm font-medium text-brand-600 hover:text-brand-700 transition-colors">
              Admin
            </Link>
          )}
        </nav>
        ...
        <div className="flex items-center gap-2">
          ...
          {user ? (
            ...
            <div className="hidden md:flex items-center gap-2">
              <Link to="/login"    className="btn-ghost !py-1.5 !px-4 text-sm">Log in</Link>
              <Link to="/register" className="btn-brand !py-1.5 !px-4 text-sm">Sign up</Link>
            </div>
          )}
            ...
            <div className="px-4 py-4 space-y-1">
              ...
              {user ? (
                ...
                <div className="flex gap-2 pt-2">
                  <Link to="/login"    onClick={() => setMenuOpen(false)} className="flex-1 btn-ghost text-sm text-center !py-2">Log in</Link>
                  <Link to="/register" onClick={() => setMenuOpen(false)} className="flex-1 btn-brand text-sm text-center !py-2">Sign up</Link>
                </div>
              )}
            </div>
          </motion.div>
