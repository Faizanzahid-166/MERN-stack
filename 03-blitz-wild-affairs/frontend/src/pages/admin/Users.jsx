...
import { adminAPI } from '../../api/APIs.js';
...
export default function Users() {
  ...
  const openConfirm = (userId, action, label) =>
    setConfirm({ open: true, userId, action, label });
  const closeConfirm = () =>
    setConfirm({ open: false, userId: null, action: null, label: '' });
      ...
      <div className="relative max-w-sm mb-6">
        ...
        <input
          ...
          className="w-full pl-10 pr-4 py-2.5 rounded-xl border border-gray-200 dark:border-gray-700
            bg-white dark:bg-gray-900 focus:outline-none focus:ring-2 focus:ring-brand-500 text-sm"
        />
      </div>
            ...
            <thead>
              <tr className="border-b border-gray-100 dark:border-gray-800 text-left bg-gray-50 dark:bg-gray-800/50">
                {['User', 'Email', 'Role', 'Joined', 'Actions'].map((h) => (
                  <th key={h} className="px-6 py-3.5 font-semibold text-xs text-gray-500 uppercase tracking-wider">
                    {h}
                  </th>
                ))}
              </tr>
                  ...
                  <td className="px-6 py-4">
                    {user.id !== me?.id ? (
                      <div className="flex items-center gap-3">
                        ...
                        <button
                          onClick={() => openConfirm(user.id, 'delete', 'Delete User')}
                          className="text-xs font-medium text-red-400 hover:text-red-600 hover:underline"
                        >
                          Delete
                        </button>
                      </div>
