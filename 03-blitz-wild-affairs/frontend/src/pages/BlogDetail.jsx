...
import { readingTime, timeAgo, formatCount } from '../utils/helpers.js';
...
export default function BlogDetail() {
  ...
  useEffect(() => {
    ...
    blogAPI
      .getBySlug(slug)
      .then(({ data }) => { setBlog(data.blog); setLikeCount(data.blog.likes ?? 0); })
      .catch(() => setBlog(null))
  ...
  if (!blog) return (
    <div className="min-h-screen flex items-center justify-center text-center px-4">
      <div>
        ...
        <Link to="/blogs" className="btn-brand mt-4 inline-block">Browse all articles</Link>
      </div>
    </div>
  );
        ...
        <div className="max-w-4xl mx-auto px-4 sm:px-6 py-12">
          ...
          <div className="flex items-center flex-wrap gap-4 mb-10 pb-8 border-b border-gray-200 dark:border-gray-800">
            ...
            <div className="ml-auto flex items-center gap-2">
              <button onClick={handleLike}
                className={`flex items-center gap-1.5 px-3 py-1.5 rounded-xl border text-sm font-medium transition-all ${liked ? 'border-red-300 bg-red-50 dark:bg-red-900/20 text-red-500' : 'border-gray-200 dark:border-gray-700 hover:text-red-500'}`}>
                {liked ? '❤️' : '🤍'} {formatCount(likeCount)}
              </button>
              <button onClick={handleBookmark}
                className={`flex items-center gap-1.5 px-3 py-1.5 rounded-xl border text-sm font-medium transition-all ${bookmarked ? 'border-brand-300 bg-brand-50 dark:bg-brand-900/20 text-brand-600' : 'border-gray-200 dark:border-gray-700 hover:text-brand-600'}`}>
                {bookmarked ? '🔖' : '📑'} Save
              </button>
            </div>
          </div>
          ...
          {blog.tags?.length > 0 && (
            <div className="flex flex-wrap gap-2 mt-10">
              {blog.tags.map((tag) => (
                <Link key={tag} to={`/blogs?search=${tag}`}><Tag>{tag}</Tag></Link>
              ))}
            </div>
          )}
            ...
            <div className="mt-12">
              ...
              <div className="space-y-5">
                {blog.blog_media.map((m) => (
                  <div key={m.id}>
                    {m.media_type === 'image' && <img src={m.media_url} alt="Attachment" className="rounded-2xl max-w-xl w-auto max-h-[600px] mx-auto shadow-md block object-contain bg-gray-50 dark:bg-gray-900/50" loading="lazy" />}
                    {m.media_type === 'video' && (
