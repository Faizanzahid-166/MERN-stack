    ...
    <div className="pt-24 pb-20 min-h-screen">
      <div className="max-w-7xl mx-auto px-4 sm:px-6">
        <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} className="text-center mb-12">
          ...
          <p className="text-gray-500 dark:text-gray-400">{loading ? '—' : `${total} article${total !== 1 ? 's' : ''} published`}</p>
        </motion.div>
          ...
          {CATEGORIES.map((cat) => {
            ...
            return (
              <button key={cat} onClick={() => setParam('category', cat === 'All' ? '' : cat)}
                className={`px-4 py-1.5 rounded-full text-sm font-medium transition-all ${active ? 'bg-brand-600 text-white shadow-md' : 'bg-gray-100 dark:bg-gray-800 text-gray-600 dark:text-gray-300 hover:bg-brand-100 hover:text-brand-600'}`}>
                {cat}
              </button>
            );
          })}
        ...
        {loading ? (
          ...
          <EmptyState onClear={() => { setSearchInput(''); setSearchParams({}); }} />
        ) : (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
