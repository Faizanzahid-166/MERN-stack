...
export const contactForm = async (req, res) => {
  try {
    ...
    const response = await resend.emails.send({
      from: "Portfolio Contact <onboarding@resend.dev>", // must be a verified domain/sender
      to: process.env.CONTACT_RECEIVER, // your email (e.g., your Gmail)
      subject: `📩 New Contact Form Submission from ${name}`,
      html: `
    ...
    return res
      .status(200)
      .json({ success: true, msg: "✅ Message sent successfully!", response });
  } catch (err) {
    console.error("❌ Contact form error:", err);
    return res.status(500).json({
      success: false,
