...
export async function getRepositoryLists(req, res) {
  try {
    const { name, project_name, limit } = req.query; // query params
    ...
    if (error) {
      console.error("Supabase fetch error:", error.message);
      return errorResponse(res, "Failed to fetch repo-lists", 500);
    }

    return successResponse(res, "repo-lists fetched successfully", { data, count });
  } catch (err) {
    console.error("Controller repo-lists error:", err.message);
    return errorResponse(res, "Server error", 500);
  }
}
